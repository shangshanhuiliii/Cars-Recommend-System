...
- 产品前端使用登录 / 注册浮窗，不使用独立登录页作为默认入口。
- 未登录访问需要身份的页面时，保持当前背景页面，通过 `auth=login` 或 `auth=admin` 打开浮窗，并保留合法 redirect。
- `/login`、`/register` 和 `/admin/login` 只作为兼容入口触发浮窗；普通用户注册只创建 USER 账号，管理员登录后默认进入 `/admin/cars`。

未登录状态不代表游客产品身份；推荐、收藏、反馈、对比和管理端能力仍必须在登录后使用 JWT 当前身份。
