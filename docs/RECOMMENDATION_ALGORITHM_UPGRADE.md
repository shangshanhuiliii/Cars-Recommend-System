# 推荐算法详细说明：主客观组合权重 + Pareto-TOPSIS

本文档是阶段 9.6 后当前主算法的详细设计和实现口径，算法版本为 `pareto-topsis-v1`。算法名称为：

```text
基于主客观组合权重与 Pareto-TOPSIS 的可解释汽车推荐算法
```

本文档维护当前主算法的公式、流程、快照口径、测试要求和论文表述。升级前的加权求和算法仅作为历史基线、TOPSIS 边界兜底 `fallbackScore` 和升级前后对比说明，相关基线说明见 `docs/RECOMMENDATION_ALGORITHM.md`。

新算法仍属于面向冷启动场景的内容特征推荐和多指标决策方法，不是深度学习、协同过滤或在线学习。

## 1. 升级背景与目标

升级前系统已经完成推荐闭环：车型参数生成特征评分，用户需求生成九维权重，推荐阶段动态计算价格分，再通过加权求和得到旧版 `totalScore`，并保存推荐理由、不足提醒、匹配状态和推荐快照。

历史规则加权求和的优点是清晰、稳定、可解释，适合第一版 MVP。但它也存在算法表达深度一般、权重主要来自用户主观偏好、候选车型客观差异利用不足的问题。加权求和还天然允许维度补偿：某个车型可能在单一维度得分很高，从而掩盖用户重点关注维度上的明显短板。
...
- 使用 TOPSIS 相对接近度替代升级前加权求和 `totalScore`。
...
升级前后端核心实现集中在 `RecommendationServiceImpl`、`UserProfileServiceImpl` 和 `CarFeatureScoreCalculator`。本节用于说明历史基线和升级对比，不表示当前主排序仍采用加权求和。
...
- 历史版本每组内部按 `totalScore desc`、`reputationScore desc`、`popularityScore desc` 排序。

历史解释与快照：

- 历史版本 `tags` 根据高分维度生成，并保存到 `recommend_item.tags`。
- 历史版本 `reasonText` 主要来自用户高权重维度和车型高分维度。
- 历史版本 `weaknessText` 主要来自用户高权重维度和车型低分维度。
...
- 历史排序算法是线性加权求和，算法表达偏基础。
- 权重主要来自用户主观表达，没有利用候选集自身的客观差异。
- 线性加权允许强补偿，可能让单一高分维度掩盖高权重维度短板。
- 历史推荐理由尚未体现与正理想解的距离，也没有说明哪些维度真正贡献了 TOPSIS 接近度。
...
其中 `v_ij` 和 `A+_j` 已经是加权归一化矩阵中的值，已经包含 `finalWeight_j`，因此不再额外乘一次权重，避免重复加权。
...
第一版已采用方案 A。理由是当前项目已经有完善的推荐快照字段，升级重点放在算法正确性、解释生成和历史不重算，而不是扩展大量中间字段。若后续答辩或论文需要展示 TOPSIS 距离，再评估方案 B。
...
升级前 `RecommendationServiceImpl` 同时承担候选生成、价格分、加权求和、标签、理由、不足、排序和快照保存。阶段 9.6 已按最短路径抽取算法组件，后续维护不应再把新增排序、解释或权重逻辑堆回编排类，但也不需要引入复杂策略框架。
...
阶段 9.6 已按以下路径完成。本节保留为后续维护、答辩复盘和风险检查依据。
